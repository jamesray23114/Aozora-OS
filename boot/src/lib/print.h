...
void print_mmap(EFI_MEMORY_DESCRIPTOR* memmap, uintn size, uintn descsize)
{
	print_num(size / descsize, 10, 1, 0);
	print_string(" memory mappings \n\n\r");

    for(int i = 0; i < size / descsize; i++)
    {
		print_string("memory ");
		print_num(i, 10, 2, ' ');
		print_string(":");

		print_memory_desc(*(EFI_MEMORY_DESCRIPTOR*)((uintn) memmap + (i * descsize)));
    }
}
